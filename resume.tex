\documentclass[a4paper, 12pt]{article}

% Packages for formatting
\usepackage[margin=1in]{geometry}
\usepackage{graphicx}
\usepackage{array}
\usepackage{enumitem}
\usepackage{xcolor}
\usepackage{tikz}
\usepackage{fontawesome}

% Header information
\newcommand{\name}{Anthony Jeanney}
\newcommand{\status}{Développeur}
\newcommand{\smalltext}{Passionné d'Informatique et de Nouvelles Technologies, titulaire d'un Master en Web et Sciences des Données, je recherche un poste de développeur afin de continuer mon parcours professionnel.}
\newcommand{\profilepic}{profile.jpg}

% Change font
\renewcommand{\familydefault}{\sfdefault}

% Define colors
\definecolor{headercolor}{RGB}{128, 128, 128}
\definecolor{sectioncolor}{RGB}{0, 102, 204}
\definecolor{itemcolor}{RGB}{0, 51, 102}

% Set margins
\geometry{left=0.7cm, right=2cm, top=0.3cm, bottom=0.5cm}

% Disable hyphenation
\hyphenpenalty=10000

% Set line spacing
\linespread{0.62}

\begin{document}

% Header
\begin{center}
	\begin{tabular}{ m{0.25\textwidth} m{1\textwidth} m{0.25\textwidth} }
		\begin{tikzpicture}
			\clip (0,0) circle (2cm);
			\node at (0,0) {\includegraphics[width = 4cm]{\profilepic}};
		\end{tikzpicture}
		\hspace{0.2cm}{\color{sectioncolor}\rule[0cm]{2pt}{3cm}}\hspace{0.2cm}
		 &
		\begin{tabular}{@{}p{0.7\textwidth}@{}}
			{\Huge \textbf{\MakeUppercase{\name}}}    \\
			\vspace{0.1cm}
			{\large \textcolor{headercolor}{\status}} \\
			\vspace{0.05cm}
			\textcolor{itemcolor}{\smalltext}
		\end{tabular}
		 &
	\end{tabular}
\end{center}

{\color{sectioncolor}\rule{\linewidth}{0.8mm}}
\vspace{0.1cm}

% Left section
\begin{minipage}[t]{0.65\textwidth}

	% Skills
	\section*{\textcolor{sectioncolor}{Compétences}}
	\begin{tabular}{ m{0.18\textwidth} m{0.73\textwidth} }
		\textbf{Langages}   & JavaScript, Typescript, C\#, Java, C, C++, Python, HTML, CSS                              \\
		                    &                                                                                           \\
		\textbf{Frameworks} & React, Vue.js, Node.js, Express, .NET                                                     \\
		                    &                                                                                           \\
		\textbf{Outils}     & Git, Docker, Nginx, Android Studio, etc.                                                  \\
		                    &                                                                                           \\
		\textbf{Autres}     & Linux, Réseaux, Bases de données, IA, Matériel Informatique, Administration Système, etc. \\
	\end{tabular}

	\vspace{-0.4cm}

	% Formations
	\section*{\textcolor{sectioncolor}{Formations}}
	\begin{tabular}{ m{0.187\textwidth} m{0.8\textwidth} }
		\textbf{2022 - 2024} & \textbf{Master Web et Sciences des Données} \\
		                     & Université du Littoral Côte d'Opale         \\
		                     & \textit{Calais, France}                     \\
		                     &                                             \\
		\textbf{2018 - 2022} & \textbf{Licence Informatique}               \\
		                     & Université du Littoral Côte d'Opale         \\
		                     & \textit{Calais, France}                     \\
		                     &                                             \\
		\textbf{2018}        & \textbf{Baccalauréat Scientifique}          \\
		                     & Lycée de l'Europe                           \\
		                     & \textit{Dunkerque, France}                  \\
	\end{tabular}

	\vspace{-0.4cm}

	% Projects
	\section*{\textcolor{sectioncolor}{Projets}}
	\begin{tabular}{ m{0.18\textwidth} m{0.73\textwidth} }
		\textbf{2024} & \textbf{Application de recherche de trains}                                                 \\
		              & \textit{Projet innovant, recherche de trains à partir de flux GTFS, en .NET et front React} \\
		              &                                                                                             \\
		              & \textbf{Application de gestion d'emploi du temps}                                           \\
		              & \textit{Projet Java JEE, avec front Vue.js}                                                 \\
		              &                                                                                             \\
		\textbf{2023} & \textbf{Application de Suivi de Carburant}                                                  \\
		              & \textit{Projet de Web Services, creation d'une API en .NET}                                 \\
		              &                                                                                             \\
		              & \textbf{IA de jeu Minecraft}                                                                \\
		              & \textit{Projet annuel de Master, en Python}                                                 \\
		              &                                                                                             \\
		              & \textbf{Application de Suivi de Santé}                                                      \\
		              & \textit{Projet de programmation agile, avec un intervenant de Decathlon}                    \\
		              &                                                                                             \\
		\textbf{2022} & \textbf{IA de jeu de Cache-Cache}                                                           \\
		              & \textit{Projet de fin d'études de Licence, en C++}                                          \\
	\end{tabular}

	\vspace{-0.4cm}

	% Experiences
	% \section*{\textcolor{sectioncolor}{Expériences}}
	% \begin{tabular}{ m{0.187\textwidth} m{0.8\textwidth} }
	% 	\textbf{2024}      & \textbf{Stage (6 mois)}           \\
	% 	                   & \textit{Développeur Full Stack}   \\
	% 	                   & \textit{AXA WebCenter, Wasquehal} \\
	% 	                   &                                   \\
	% 	\textbf{2021-2023} & \textbf{Travail Saisonnier}       \\
	% 	                   & \textit{Agent Outils}             \\
	% 	                   & \textit{Dillinger France}         \\
	% 	                   &                                   \\
	% 	\textbf{2019}      & \textbf{Travail Saisonnier}       \\
	% 	                   & \textit{Traitement de Données}    \\
	% 	                   & \textit{Capdune, Dunkerque}       \\
	% 	%                    &                                         \\
	% 	% \textbf{2015}      & \textbf{Stage Pratique}                 \\
	% 	%                    & \textit{Animateur Scientifique}         \\
	% 	%                    & \textit{Planétarium, Capelle-la-Grande} \\
	% 	%                    &                                         \\
	% 	% \textbf{2014}      & \textbf{Stage Pratique}                 \\
	% 	%                    & \textit{Développeur}                    \\
	% 	%                    & \textit{Aurea, Lille}                   \\
	% \end{tabular}

\end{minipage}
% Right section
\begin{minipage}[t]{0.35\textwidth}

	% Information
	\section*{\textcolor{sectioncolor}{Informations}}
	\begin{tabular}{ m{0.07\textwidth} m{0.9\textwidth} }
		\textbf{\faPhone}        & (+33) 7 82 21 56 72       \\
		                         &                           \\
		\textbf{\faEnvelope}     & anthony.jeanney@gmail.com \\
		                         &                           \\
		\textbf{\faGlobe}        & anthony.jeanney.fr        \\
		                         &                           \\
		\textbf{\faMapMarker}    & Lille / Dunkerque         \\
		                         & France                    \\
		                         &                           \\
		\textbf{\faBirthdayCake} & 17/06/2000                \\
		                         &                           \\
		\textbf{\faCar}          & Permis B / Véhiculé       \\
	\end{tabular}

	% Languages
	\section*{\textcolor{sectioncolor}{Langues}}
	\begin{tabular}{ m{0.3\textwidth} m{0.6\textwidth} }
		\textbf{Français} & \textit{Voltaire : 577/900}            \\
		                  & \textit{(Orthographe Professionnelle)} \\
		                  &                                        \\
		\textbf{Anglais}  & Niveau B2 / C1                         \\
		                  &                                        \\
		                  & \textit{CLES B2}                       \\
		                  &                                        \\
		\textbf{Allemand} & Niveau A2
	\end{tabular}

	% Hobbies
	\section*{\textcolor{sectioncolor}{Loisirs}}
	\begin{itemize}[leftmargin=0.5cm, label={\textcolor{itemcolor}{\faAngleRight}}]
		\item Programmation
		\item Domotique
		\item IoT
		\item Électronique
		\item Réseaux
		      % \item Assemblage de Matériel\\ Informatique
		\item Jeux Vidéos
	\end{itemize}

	% Personnnality
	\section*{\textcolor{sectioncolor}{Personnalité}}
	\begin{tabular}{ m{0.3\textwidth} m{0.6\textwidth} }
		\textbf{Curieux}  & \\ & \\
		\textbf{Autonome} & \\ & \\
		\textbf{Sociable} &
	\end{tabular}

\end{minipage}

\vspace{-0.3cm}


% Experiences
\begin{minipage}[t]{1\textwidth}
	\section*{\textcolor{sectioncolor}{Expériences}}
	\begin{tabular}{ m{0.11\textwidth} m{0.35\textwidth} m{0.11\textwidth} m{0.35\textwidth} }
		\textbf{2024-Present} & \textbf{Développeur Consultant}              & \textbf{2021-2023} & \textbf{Agent Outils}                \\
		                      & \textit{Full Stack PHP, Java, JavaScript...} &                    & \textit{Travail Saisonnier}          \\
		                      & \textit{Talan, Lille/Amiens}                 &                    & \textit{Dillinger France, Dunkerque} \\
		                      &                                              &                    &                                      \\
		\textbf{2024}         & \textbf{Développeur Full Stack}              & \textbf{2019}      & \textbf{Traitement de Données}       \\
		                      & \textit{Stage de fin d'études}               &                    & \textit{Travail Saisonnier}          \\
		                      & \textit{AXA WebCenter, Wasquehal}            &                    & \textit{Capdune, Dunkerque}          \\
	\end{tabular}
\end{minipage}

\end{document}
